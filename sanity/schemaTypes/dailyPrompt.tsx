  ...
  fields: [
    defineField({
      ...
      validation: (Rule) => [
        ...
        Rule.max(60).warning('Keep title under 60 characters for better display'),
      ],
    }),
    defineField({
      name: 'prompt',
      ...
      type: 'text',
      rows: 3,
      description: 'Thea actual prompt text that will inspire journal entries',
      validation: (Rule) => [
        Rule.required().error('Prompt texst is required'),
        Rule.min(10).error('Prompt should be at least 10 characters'),
        Rule.max(200).warning('Consider keeping prompts under 200 characters'),
      ],
    }),
    defineField({
      name: 'emoji',
    ...
    defineField({
      ...
      title: 'Weight',
      type: 'number',
      description: 'Higher weights increase the change of this prompt being selected (1-10)',
      initialValue: 5,
      validation: (Rule) => [
